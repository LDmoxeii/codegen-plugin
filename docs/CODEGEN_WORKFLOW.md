...
根据代码分析，你的简介基本正确，但有以下**修正和补充**：

#### ✅ 正确的部分

1. **基础上下文构建** - 正确
   - 确实通过 `AbstractCodegenTask.baseMap` 构建
   - 包含项目信息、模块路径、数据库配置、生成配置

2. **GenArch 模板加载** - 正确
   - 确实在 `loadTemplate()` 中反序列化 JSON
   - 确实分为脚手架节点（PathNode）和模板节点（TemplateNode）
   - 确实有初步解析（`template.resolve(baseMap)`）

3. **GenEntity 重复步骤2** - 正确
   - 确实设置了 `renderFileSwitch = false`
   - 确实重新调用 `super.generate()` 执行 `render()`

4. **GenEntity 上下文构建** - 正确
   - 8 个 ContextBuilder 顺序正确
   - 9 个 Generator 顺序正确

5. **GenAnnotation 基本流程** - 正确
   - 确实读取 KSP 生成的 JSON 元数据
   - 确实通过 ContextBuilder 构建上下文
   - 确实通过 Generator 生成文件

#### ❌ 需要修正的部分

**修正 1：GenArch 的 renderTemplate()**

你说："并通过节点信息，缓存一些额外的上下文，例如 aggregatesPackage等。主要通过renderTemplate抽象方法，由具体task定义"

**实际情况**：
- `renderTemplate()` 在 GenArch 阶段**不做任何事**（空实现）
- 只有在 GenEntity 阶段，`renderTemplate()` 才缓存路径和模板节点
- `aggregatesPackage` 等包名是通过 lazy 计算的，不是在 renderTemplate 中缓存

**正确描述**：
> GenArch 阶段只渲染脚手架文件，不缓存上下文。GenEntity 阶段重新执行 render()，此时通过 renderTemplate() 缓存模板节点到 templateNodeMap，并记录目录路径（aggregatesPath、schemaPath等）。

**修正 2：模板解析的时机**

你说："这时候由于只有基础上下文，所有模板节点的文本有可能没有解析完全"

**实际情况**：
- **脚手架节点（PathNode）** 在 GenArch 阶段完全解析并写入文件
- **模板节点（TemplateNode）** 不在 GenArch 阶段使用，而是缓存起来供 GenEntity 使用
- GenEntity 为每个表深拷贝模板节点，用表级上下文重新解析

**正确描述**：
> GenArch 阶段完全解析脚手架节点（用于生成目录结构和基础文件）。模板节点不在此阶段使用，而是缓存到 templateNodeMap。GenEntity 阶段为每个表深拷贝模板节点，用包含实体信息的完整上下文重新解析，生成实体特定的代码。

**修正 3：renderFileSwitch 的作用**

你说："然后通过 AbstractCodegenTask 中的 render 方法写入脚手架文件"

**实际情况**：
- GenArch 阶段：`renderFileSwitch = true`（默认），写入脚手架文件
- GenEntity 阶段：`renderFileSwitch = false`，只缓存路径，不写入文件
- GenEntity 生成实体时：通过 `forceRender()` 强制写入，无视 `renderFileSwitch`

**正确描述**：
> GenArch 阶段写入脚手架文件。GenEntity 阶段先设置 renderFileSwitch = false 重新执行 render() 缓存路径，然后在生成实体时通过 forceRender() 强制写入文件。

**修正 4：GenAnnotation 的前置条件**

你说："当通过GenArch、GenEntity 生成基础框架后（主要是框架以及核心实体类），可以调用 GenAnnotation"

**实际情况**：
- GenAnnotation **依赖 KSP Processor** 生成的 JSON 元数据
- KSP Processor 需要编译 Domain 层代码才能执行
- 因此流程是：`GenArch → GenEntity → 编译 → KSP → GenAnnotation`

**正确描述**：
> 完整流程：
> 1. GenArch 生成脚手架
> 2. GenEntity 生成实体类
> 3. **编译项目**（触发 KSP Processor）
> 4. KSP 生成元数据 JSON
> 5. GenAnnotation 读取 JSON 生成基础设施代码
...
1. **基础上下文构建**（所有任务前）
   - 通过 `AbstractCodegenTask.baseMap` 构建基础上下文
   - 包含项目信息、模块路径、数据库配置、生成配置（40+ 项）
   - 定义模板别名系统（300+ 映射），支持多种命名风格

2. **GenArch - 脚手架生成**
   - 初始化 Pebble 模板引擎
   - 加载架构模板 JSON，反序列化为 Template 对象
     - Template 包含脚手架节点（PathNode）和模板节点（TemplateNode）
   - 使用基础上下文初步解析模板（`template.resolve(baseMap)`）
   - 渲染脚手架节点，生成项目目录结构和基础文件
   - 模板节点不在此阶段使用，保留给 GenEntity

3. **GenEntity - 实体生成**

   **3.1 模板缓存阶段**
   - 设置 `renderFileSwitch = false`
   - 重新执行 `super.generate()` → `render()`
   - 通过 `renderTemplate()` 缓存：
     - 模板节点到 `templateNodeMap`（tag → List<TemplateNode>）
     - 目录路径（aggregatesPath、schemaPath、subscriberPath）

   **3.2 上下文构建阶段**
   - 通过 8 个 ContextBuilder 按顺序构建 EntityContext：
     - TableContextBuilder (order=10) - 表和列信息
     - EntityTypeContextBuilder (order=20) - 实体类型
     - AnnotationContextBuilder (order=20) - 注解信息
     - ModuleContextBuilder (order=20) - 模块信息
   ...
   **3.3 文件生成阶段**
   ...
   - 每个 Generator 为每个表：
     - 构建表级上下文（扩展基础上下文）
     - 深拷贝模板节点，用表级上下文重新解析
     - 通过 `forceRender()` 强制写入文件
     - 调用 `onGenerated()` 缓存类型映射到 `typeMapping`

4. **KSP Processor - 编译时注解处理**（中间步骤）
   - 用户执行 `./gradlew build` 编译项目
   - KSP Processor 在编译时扫描 @Aggregate、@Entity 注解
   - 生成元数据 JSON 到 `build/generated/ksp/main/resources/metadata/`
     - aggregates.json - 聚合根和实体信息
     - entities.json - JPA 实体信息

5. **GenAnnotation - 注解代码生成**

   **5.1 元数据加载**
   - 解析 KSP 元数据路径（优先 domain 模块）
   - 读取 aggregates.json 和 entities.json

   **5.2 上下文构建**
   - 通过 3 个 AnnotationContextBuilder 构建 AnnotationContext：
     - KspMetadataContextBuilder (order=10) - 读取 JSON 元数据
     - AggregateInfoBuilder (order=20) - 识别聚合根，组织聚合结构
     - IdentityTypeBuilder (order=30) - 解析 ID 类型，填充 typeMapping

   **5.3 文件生成**
   - 通过 AnnotationTemplateGenerator 按聚合生成代码：
     - RepositoryGenerator (order=10) - Repository 接口
     - (ServiceGenerator - 已排除)
     - (ControllerGenerator - 未实现)
   - 每个 Generator 为每个聚合：
     - 构建聚合级上下文（扩展基础上下文）
     - 深拷贝模板节点，用聚合级上下文解析
     - 通过 `forceRender()` 写入文件
